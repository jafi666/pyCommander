...
class WindowMenuBar(QtGui.QMenuBar):
    ...
    def setup_menu_bar_file(self):
        ...
        action_new_file = self.create_menu_bar_action("New File", "Ctrl+N", "Create a new file", 
                                                      self.commander_window.file_manager.add_new_file)
        self.menuFile.addAction(action_new_file)
        action_quit = self.create_menu_bar_action("Quit", "Ctrl+Q", "Exit from Application", 
                                                  QtGui.qApp.quit)
    ...
    def setup_menu_bar_help(self):
        ...
        action_about = self.create_menu_bar_action("About PyComander...", None, 
                                                   "Review About PyCommander Information")
    ...
    def create_menu_bar_action(self, text, shortcut = None, tip = "", connection = None):
        ...
        Keyword arguments:
        text -- text to be set in the action (ie. "File")
        shortcut -- string sequence of keys or single key for shortcut (ie. "Crtl+Q", default None)
        tip -- string to set a tool tip (ie. "System File Menu button", default empty)
